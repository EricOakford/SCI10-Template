...
;;;	)
	...
	(method (play)
		...
		(self 
			...
			; Yeah, right, guys!  Like, just ignore the waitCursor completely!
	...
	(method (doit &tmp event evtTime)
		...
		(if fastCast 
			...
			(while fastCast
				...
				(GetEvent allEvents talkerEvent)
				(if (and 
						(talkerEvent type?)
						fastCast	
						(or
							(> gameTime evtTime)
							;; roll over
							(> (Abs (- evtTime gameTime)) $7000)
						)
					)
					(fastCast firstTrue:#handleEvent talkerEvent)
				)
;				(ListOps LEachElementDo (fastCast elements?) #doit:)
			)
		else
